...
static IRAM_ATTR void TimerCallback (TimerHandle_t xTimer) {
    if (TimerEnabled) {
        TimerTickCounter++;
...
void TimerHwInit( void ) {
    ...
    // TODO: This is so sad... but let's avoid complications first
    if(LoRaTimer == NULL){
      LoRaTimer = xTimerCreate("LoRa_Timer", 1 / portTICK_PERIOD_MS, pdTRUE, (void *) 0, TimerCallback);
    }
