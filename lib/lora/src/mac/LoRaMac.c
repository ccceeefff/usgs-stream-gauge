...
//minimum number of enabled channels. by specification should be 2.
//for experiment purposes set as 1
#define MIN_COUNT_ENABLED_125KHZ_CHANNELS           1

/*!
 * Device IEEE EUI
 */
static RTC_DATA_ATTR uint8_t *LoRaMacDevEui;

/*!
 * Application IEEE EUI
 */
static RTC_DATA_ATTR uint8_t *LoRaMacAppEui;

/*!
 * AES encryption/decryption cipher application key
 */
static RTC_DATA_ATTR uint8_t *LoRaMacAppKey;

/*!
 * AES encryption/decryption cipher network session key
 */
static RTC_DATA_ATTR uint8_t LoRaMacNwkSKey[] =
{
 ...
 */
static RTC_DATA_ATTR uint8_t LoRaMacAppSKey[] =
{
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
};
 ...
 * measurements
 */
static RTC_DATA_ATTR uint16_t LoRaMacDevNonce;

/*!
 * Network ID ( 3 bytes )
 */
static RTC_DATA_ATTR uint32_t LoRaMacNetID;

/*!
 * Mote Address
 */
static RTC_DATA_ATTR uint32_t LoRaMacDevAddr;

/*!
 * Multicast channels linked list
 */
static RTC_DATA_ATTR MulticastParams_t *MulticastChannels = NULL;

/*!
 * Actual device class
 */
static RTC_DATA_ATTR DeviceClass_t LoRaMacDeviceClass;

/*!
 * Indicates if the node is connected to a private or public network
 */
static RTC_DATA_ATTR bool PublicNetwork;

/*!
 * Indicates if the node supports repeaters
 */
static RTC_DATA_ATTR bool RepeaterSupport;

/*!
 * Buffer containing the data to be sent or received.
 */
static RTC_DATA_ATTR uint8_t LoRaMacBuffer[LORAMAC_PHY_MAXPAYLOAD];

/*!
 * Length of packet in LoRaMacBuffer
 */
static RTC_DATA_ATTR uint16_t LoRaMacBufferPktLen = 0;

/*!
 * Length of the payload in LoRaMacBuffer
 */
static RTC_DATA_ATTR uint8_t LoRaMacTxPayloadLen = 0;

/*!
 * Buffer containing the upper layer data.
 */
static RTC_DATA_ATTR uint8_t LoRaMacRxPayload[LORAMAC_PHY_MAXPAYLOAD];

/*!
 * LoRaMAC frame counter. Each time a packet is sent the counter is incremented.
 * Only the 16 LSB bits are sent
 */
static RTC_DATA_ATTR uint32_t UpLinkCounter = 0;

/*!
 * LoRaMAC frame counter. Each time a packet is received the counter is incremented.
 * Only the 16 LSB bits are received
 */
static RTC_DATA_ATTR uint32_t DownLinkCounter = 0;

/*!
 * IsPacketCounterFixed enables the MIC field tests by fixing the
 * UpLinkCounter value
 */
static RTC_DATA_ATTR bool IsUpLinkCounterFixed = false;

/*!
 * Used for test purposes. Disables the opening of the reception windows.
 */
static RTC_DATA_ATTR bool IsRxWindowsEnabled = true;

/*!
 * Indicates if the MAC layer has already joined a network.
 */
static RTC_DATA_ATTR bool IsLoRaMacNetworkJoined = false;

/*!
 * LoRaMac ADR control status
 */
static RTC_DATA_ATTR bool AdrCtrlOn = false;

/*!
 * Counts the number of missed ADR acknowledgements
 */
static RTC_DATA_ATTR uint32_t AdrAckCounter = 0;

/*!
 * If the node has sent a FRAME_TYPE_DATA_CONFIRMED_UP this variable indicates
 * if the nodes needs to manage the server acknowledgement.
 */
static RTC_DATA_ATTR bool NodeAckRequested = false;

/*!
 * If the server has sent a FRAME_TYPE_DATA_CONFIRMED_DOWN this variable indicates
 * if the ACK bit must be set for the next transmission
 */
static RTC_DATA_ATTR bool SrvAckRequested = false;

/*!
 * Indicates if the MAC layer wants to send MAC commands
 */
static RTC_DATA_ATTR bool MacCommandsInNextTx = false;

/*!
 * Contains the current MacCommandsBuffer index
 */
static RTC_DATA_ATTR uint8_t MacCommandsBufferIndex = 0;

/*!
 * Contains the current MacCommandsBuffer index for MAC commands to repeat
 */
static RTC_DATA_ATTR uint8_t MacCommandsBufferToRepeatIndex = 0;

/*!
 * Buffer containing the MAC layer commands
 */
static RTC_DATA_ATTR uint8_t MacCommandsBuffer[LORA_MAC_COMMAND_MAX_LENGTH];

/*!
 * Buffer containing the MAC layer commands which must be repeated
 */
static RTC_DATA_ATTR uint8_t MacCommandsBufferToRepeat[LORA_MAC_COMMAND_MAX_LENGTH];
 ...
 * LoRaMac bands
 */
static RTC_DATA_ATTR Band_t Bands[LORA_MAX_NB_BANDS] =
{
    BAND0,
};
 ...
 * LoRaMAC channels
 */
static RTC_DATA_ATTR ChannelParams_t Channels[LORA_MAX_NB_CHANNELS];

/*!
 * Contains the channels which remain to be applied.
 */
static RTC_DATA_ATTR uint16_t ChannelsMaskRemaining[6];
 ...
 */
static RTC_DATA_ATTR uint8_t ChannelsNbRepCounter = 0;

/*!
 * Maximum duty cycle
 * \remark Possibility to shutdown the device.
 */
static RTC_DATA_ATTR uint8_t MaxDCycle = 0;

/*!
 * Aggregated duty cycle management
 */
static RTC_DATA_ATTR uint16_t AggregatedDCycle;
static RTC_DATA_ATTR TimerTime_t AggregatedLastTxDoneTime;
static RTC_DATA_ATTR TimerTime_t AggregatedTimeOff;

/*!
 * Enables/Disables duty cycle management (Test only)
 */
static RTC_DATA_ATTR bool DutyCycleOn;

/*!
 * Current channel index
 */
static RTC_DATA_ATTR uint8_t Channel;
 ...
 */
static RTC_DATA_ATTR TimerTime_t LoRaMacInitializationTime = 0;
 ...
 */
uint32_t RTC_DATA_ATTR LoRaMacState = LORAMAC_IDLE;
 ...
 */
static RTC_DATA_ATTR uint32_t RxWindow1Delay;
static RTC_DATA_ATTR uint32_t RxWindow2Delay;
 ...
 * Rx windows params
 */
static RTC_DATA_ATTR RxConfigParams_t RxWindowsParams[2];

/*!
 * Acknowledge timeout timer. Used for packet retransmissions.
 */
static RTC_DATA_ATTR TimerEvent_t AckTimeoutTimer;

/*!
 * Number of trials to get a frame acknowledged
 */
static RTC_DATA_ATTR uint8_t AckTimeoutRetries = 1;

/*!
 * Number of trials to get a frame acknowledged
 */
static RTC_DATA_ATTR uint8_t AckTimeoutRetriesCounter = 1;

/*!
 * Indicates if the AckTimeout timer has expired or not
 */
static RTC_DATA_ATTR bool AckTimeoutRetry = false;
 ...
 */
static RTC_DATA_ATTR uint8_t JoinRequestTrials;

/*!
 * Maximum number of trials for the Join Request
 */
static RTC_DATA_ATTR uint8_t MaxJoinRequestTrials;
 ...
 */
static RTC_DATA_ATTR uint8_t RxSlot = 0;
 ...
 */
static void OnRadioTxDone( uint64_t timestamp );
 ...
 */
static void OnRadioRxDone( uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr, uint64_t timestamp );
 ...
 */
bool ValidateDatarate( int8_t datarate, uint16_t* channelsMask );
...
static void OnRadioTxDone( uint64_t timestamp )
{
    ...
    McpsConfirm.Timestamp = timestamp;
...
static void OnRadioRxDone( uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr, uint64_t timestamp )
{
    ...
    McpsIndication.RxTimeOnAir = Radio.TimeOnAir(MODEM_LORA, size);
    McpsIndication.Timestamp = timestamp;

    if(RxSlot == 0){
      McpsIndication.RxDelay = LoRaMacParams.ReceiveDelay1;
    } else {
      McpsIndication.RxDelay = LoRaMacParams.ReceiveDelay2;
    }
...
static bool RxWindowSetup( uint32_t freq, int8_t datarate, uint32_t bandwidth, uint16_t timeout, bool rxContinuous )
{
    ...
    RadioModems_t modem;
    ESP_LOGD("[loramac]", "Recv: frequency: %d | datarate: %d | bandwidth: %d | timeout: %d", freq, datarate, bandwidth, timeout);
            ...
            case SRV_MAC_LINK_ADR_REQ:
                {
                    ...
                    else
                    {
                        ...
                        if( ( datarate < DR_4 ) && ( CountNbEnabled125kHzChannels( channelsMask ) < MIN_COUNT_ENABLED_125KHZ_CHANNELS ) )
                        {
                            status &= 0xFE; // Channel mask KO
                        }
...
static void CalculateBackOff( uint8_t channel )
{
    ...
    if( IsLoRaMacNetworkJoined == false )
    {
        ...
        Bands[Channels[channel].Band].TimeOff = (TxTimeOnAir * dutyCycle - TxTimeOnAir)/1000;
    }
    else
    {
        if( DutyCycleOn == true )
    ...
    AggregatedTimeOff = AggregatedTimeOff + ( TxTimeOnAir * AggregatedDCycle - TxTimeOnAir )/1000;
}
...
LoRaMacStatus_t SendFrameOnChannel( ChannelParams_t channel )
{
    ...
    ESP_LOGD("[loramac]", "Send: frequency: %d | datarate: %d | txpower: %d", channel.Frequency, LoRaMacParams.ChannelsDatarate, txPowerIndex);
...
{
    ESP_LOGI("loramac", "initializing loramac! join state: %d", IsLoRaMacNetworkJoined);
    ...
    switch( mibSet->Type )
    {
        ...
        case MIB_CHANNELS_DEFAULT_MASK:
        {
            if( mibSet->Param.ChannelsDefaultMask )
            {
                ...
                if( chanMaskState == true )
                {
                    uint8_t CountNbEnabled = CountNbEnabled125kHzChannels( mibSet->Param.ChannelsMask );
                    if( ( CountNbEnabled < MIN_COUNT_ENABLED_125KHZ_CHANNELS ) &&
                        ( CountNbEnabled > 0 ) )
                    {
                        status = LORAMAC_STATUS_PARAMETER_INVALID;
                    }
        ...
        case MIB_CHANNELS_MASK:
        {
            if( mibSet->Param.ChannelsMask )
            {
                ...
                if( chanMaskState == true )
                {
                    if( ( CountNbEnabled125kHzChannels( mibSet->Param.ChannelsMask ) < MIN_COUNT_ENABLED_125KHZ_CHANNELS ) &&
...
LoRaMacStatus_t LoRaMacMcpsRequest( McpsReq_t *mcpsRequest )
{
    ...
    switch( mcpsRequest->Type )
    {
        case MCPS_UNCONFIRMED:
        {
            ...
            // LoRaMacParams.ChannelsTxPower = mcpsRequest->Req.Unconfirmed.TxPower;
            break;
        }
        case MCPS_CONFIRMED:
        {
            ...
            // LoRaMacParams.ChannelsTxPower = mcpsRequest->Req.Confirmed.TxPower;
            break;
        }
