 ...
 */
  ...
  */
 static void SX1276OnDioIrq (void *arg);

/*!
 * \brief DIO 0 IRQ callback
 */
void SX1276OnDio0Irq( uint64_t timestamp );
...
static void TaskLoRaIrq(void *arg);
static void SX1276OnDioIrqImpl(uint64_t timestamp);
...
void SX1276Init( RadioEvents_t *events )
{
    ...
    // create task before irq is allocated
    xTaskCreate(TaskLoRaIrq, "LoraIrq", configMINIMAL_STACK_SIZE*4, NULL, 12, NULL);
    ...
    case MODEM_LORA:
        {
            ...
            // return ms secs
            airTime = floor( tOnAir * 1e6 + 0.999999 );
        }
        break;
    }
    return airTime;
    ...
    case MODEM_LORA:
        {
            ...
            uint8_t state = ( SX1276Read( REG_OPMODE ) );
            ESP_LOGW(TAG, "send --- tx radio state: 0x%X", state);
            ...
            state = ( SX1276Read( REG_OPMODE ) );
            ESP_LOGW(TAG, "send --- tx radio should be in standby state: 0x%X", state);
            // Write payload buffer
...
void SX1276SetTx( uint32_t timeout )
{
    ESP_LOGD(TAG, "Setting tx timeout: %d", timeout);
    ...
    uint8_t state = ( SX1276Read( REG_OPMODE ) );
    ESP_LOGW(TAG, "tx radio state: 0x%X", state);
}
...
void SX1276Reset( void )
{
#if LORA_RADIO_RESET_ENABLED
  board_lora_enable(false);
  // Wait 1 ms
  DelayMs( 1 );
  board_lora_enable(true);
  // Wait 6 ms
  DelayMs( 6 );
#endif
}
...
void SX1276SetOpMode( uint8_t opMode )
{
    ...
    uint8_t state = ( SX1276Read( REG_OPMODE ) & RF_OPMODE_MASK );
    // add a small delay here to properly set chip to sleep
    DelayMs(1);
    SX1276Write( REG_OPMODE, state | opMode );
}

IRAM_ATTR void SX1276SetModem( RadioModems_t modem )
{
    assert( ( SX1276.Spi.Spi != NULL ) );
...
{
    esp_err_t err = SpiInOutBuffer(&SX1276.Spi, SPI_CMD_WRITE, addr, buffer, size, NULL, 0);
    if(err != ESP_OK){
      ESP_LOGE(TAG, "SPI write error: %d", err);
    }
}

IRAM_ATTR void SX1276ReadBuffer( uint8_t addr, uint8_t *buffer, uint8_t size )
{
    esp_err_t err = SpiInOutBuffer(&SX1276.Spi, SPI_CMD_READ, addr, NULL, 0, buffer, size);
    if(err != ESP_OK){
      ESP_LOGE(TAG, "SPI read error: %d", err);
    }
}
...
IRAM_ATTR void SX1276OnTimeoutIrq( void )
{
    ...
    case RF_TX_RUNNING:
        ...
        // Reset the radio
        ESP_LOGE(TAG, "tx timeout.. resetting radio");

        uint8_t irqflags1 = SX1276Read(REG_LR_IRQFLAGS);
        uint8_t irqflagsMask = SX1276Read(REG_LR_IRQFLAGSMASK);

        ESP_LOGW(TAG, "radio irq mask: 0x%X flags: 0x%X", irqflagsMask, irqflags1);

        SX1276Reset( );

        // Calibrate Rx chain
        ESP_LOGW(TAG, "rx chain calibration");
        RxChainCalibration( );

        // Initialize radio default values
        ESP_LOGW(TAG, "setting radio to sleep");
        uint8_t state = ( SX1276Read( REG_OPMODE ) );
        ESP_LOGW(TAG, "current radio state: 0x%X", state);
        SX1276SetOpMode( RF_OPMODE_SLEEP );

        // check op mode
        state = ( SX1276Read( REG_OPMODE ) );
        ESP_LOGW(TAG, "new radio state: 0x%X", state);
        ...
        ESP_LOGW(TAG, "setting public network");
...
// TODO: clean this up for proper abstraction and documentation
QueueHandle_t _loraIrqQueue;
static void TaskLoRaIrq(void *arg){
  uint64_t ts = 0;
  _loraIrqQueue = xQueueCreate(5, sizeof(uint64_t));
  while(1){
    if(xQueueReceive(_loraIrqQueue, &ts, portMAX_DELAY)){
      SX1276OnDioIrqImpl(ts);
    }
  }
}

static IRAM_ATTR void SX1276OnDioIrq (void *arg) {
  uint64_t ts = 0;
  if(arg){
      uint64_t *arg64 = (uint64_t *)arg;
      ts = *arg64;
  }
  xQueueSendToBackFromISR(_loraIrqQueue, &ts, NULL);
}

static IRAM_ATTR void SX1276OnDioIrqImpl (uint64_t ts) {
    if (SX1276.Settings.State > RF_IDLE) {
        ...
        switch (SX1276.Settings.Modem) {
        case MODEM_FSK:
            irqflags1 = SX1276Read(REG_IRQFLAGS1);
            irqflags2 = SX1276Read(REG_IRQFLAGS2);
            if (SX1276.Settings.State == RF_TX_RUNNING) {
            ...
            } else if (SX1276.Settings.State == RF_RX_RUNNING) {
                ...
                if (irqflags1 & RF_IRQFLAGS1_SYNCADDRESSMATCH) {
                    SX1276OnDio2Irq();
                    // clear the irq
                    SX1276Write( REG_IRQFLAGS1, RF_IRQFLAGS1_SYNCADDRESSMATCH);
                }
        ...
        case MODEM_LORA:
            ...
            ESP_LOGV(TAG, "OnDioIRQ: %d", irqflags1);
...
IRAM_ATTR void SX1276OnDio0Irq( uint64_t ts )
{
    volatile uint8_t irqFlags = 0;
    ESP_LOGV(TAG, "OnDio0IRQ: %llu", ts);
    switch( SX1276.Settings.State )
    {
        case RF_RX_RUNNING:
            //TimerStop( &RxTimeoutTimer );
            ...
            case MODEM_FSK:
                ...
                {
                    RadioEvents->RxDone( RxTxBuffer, SX1276.Settings.FskPacketHandler.Size, SX1276.Settings.FskPacketHandler.RssiValue, 0, ts );
                }
                SX1276.Settings.FskPacketHandler.PreambleDetected = false;
                SX1276.Settings.FskPacketHandler.SyncWordDetected = false;
                SX1276.Settings.FskPacketHandler.NbBytes = 0;
            ...
            case MODEM_LORA:
                {
                    ...
                    if( ( RadioEvents != NULL ) && ( RadioEvents->RxDone != NULL ) )
                    {
                        RadioEvents->RxDone( RxTxBuffer, SX1276.Settings.LoRaPacketHandler.Size, SX1276.Settings.LoRaPacketHandler.RssiValue, SX1276.Settings.LoRaPacketHandler.SnrValue, ts );
                    }
                }
                break;
            default:
