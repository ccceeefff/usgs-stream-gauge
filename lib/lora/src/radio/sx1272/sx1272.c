...
static void SX1272OnDioIrqImpl(void);
...
void SX1272Init( RadioEvents_t *events )
{
    ...
    xTaskCreate(TaskLoRaIrq, "LoraIrq", configMINIMAL_STACK_SIZE*4, NULL, 5, NULL);
    ...
    case MODEM_LORA:
        {
            ...
            // return msecs
            airTime = floor( tOnAir * (double)1e6 + (double)0.999999 );
        }
        break;
    }
    return airTime;
...
void SX1272SetOpMode( uint8_t opMode )
{
    ...
    uint8_t state = ( SX1272Read( REG_OPMODE ) & RF_OPMODE_MASK );
    // add a small delay here to properly set chip to sleep
    DelayMs(1);
    SX1272Write( REG_OPMODE, state | opMode );
}
...
{
    SpiInOutBuffer(&SX1272.Spi, SPI_CMD_WRITE, addr, buffer, size, NULL, 0);
}

IRAM_ATTR void SX1272ReadBuffer( uint8_t addr, uint8_t *buffer, uint8_t size )
{
    SpiInOutBuffer(&SX1272.Spi, SPI_CMD_READ, addr, NULL, 0, buffer, size);
}
...
static void TaskLoRaIrq(void *arg){
  char dummy = 'a';
  _loraIrqQueue = xQueueCreate(5, sizeof(char));
  while(1){
    if(xQueueReceive(_loraIrqQueue, &dummy, portMAX_DELAY)){
      SX1272OnDioIrqImpl();
    }
  }
}
...
static IRAM_ATTR void SX1272OnDioIrq (void) {
  char dummy = 'b';
  xQueueSendToBackFromISR(_loraIrqQueue, &dummy, NULL);
}

static IRAM_ATTR void SX1272OnDioIrqImpl (void) {
    if (SX1272.Settings.State > RF_IDLE) {
        ...
        case MODEM_LORA:
            ...
            ESP_LOGD("lora irq", "irq handle %d", irqflags1);
