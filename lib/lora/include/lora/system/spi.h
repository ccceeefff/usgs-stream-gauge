 ...
 *
 * \param [IN] obj      SPI object
 * \param [IN] txBuffer buffer of bytes to send
 * \param [IN] txBytes  number of bytes to send
 * \param [IN] rxBuffer buffer for receiving bytes
 * \param [IN] rxBytes  number of bytes to receive
 * \retval esp_err_t    error value, if any
 */
int SpiInOutBuffer(Spi_t *obj, uint16_t command, uint64_t address, void *txBuffer, uint32_t txBytes, void *rxBuffer, uint32_t rxBytes);
